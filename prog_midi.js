...
sendTask.interval = tempo; // milliseconds between chords
...
function progMidi(chord, genre, initialDelay) {
    ...
    // Default delay of 2000ms (2 seconds) if not specified
    ...
    isFirstRun = true; // Mark this as the first run through the progression
    
    // First play the input chord immediately
    outlet(0, midiProgression[0]);
    
    // Then wait for the specified delay before starting the progression
    initialDelayTask.schedule(initialDelay);
}

// Function to start the progression after initial delay
function startProgression() {
    currentIndex = 1; // Start with the second chord since we already played the first
...
// Function called by Task to send each chord
function sendNextChord() {
    ...
    if (currentIndex < midiProgression.length) {
        ...
        // We've reached the end of the progression
        currentIndex = 0;
        outlet(0, midiProgression[currentIndex]);
        currentIndex++;
    }
...
// Function to stop the progression - exposed for use in Max/MSP
function stop() {
    isLooping = false;
    sendTask.cancel();
    initialDelayTask.cancel();
...
function setTempo(temposlider) {
  tempo = temposlider; // Make the seed bigger
}
